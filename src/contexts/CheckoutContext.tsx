...
export function CheckoutProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onVisibility = () => { if (document.visibilityState === 'visible') unlock(); };
  ...
  const startCheckout = useCallback(async (cartId: string): Promise<boolean> => {
    ...
    try {
      ...
      setState(s => ({
        ...
        customer: (data.customer as CheckoutCustomer) ?? null,
        reverseCharge: !!data.reverse_charge,
        vatText: data.vat_text ?? null,
        vatRegime: data.vat_regime ?? null,
        currentStep: 1,
        isLoading: false,
        errors: {},
        fieldErrors: {},
